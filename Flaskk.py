        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE estado = 'Pendiente'
                  AND fecha >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp
                  AND fecha <  (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp + interval '1 day'
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                FROM formulario
                WHERE fecha >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp
                  AND fecha <  (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp + interval '1 day'
...
def get_corte_reporte(current_user):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COALESCE(SUM(precio*cantidad),0) AS total FROM formulario WHERE fecha >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp AND fecha < (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp + interval '1 day' AND metodo_pago='Efectivo'")
            efectivo = float(cur.fetchone()['total'])
            cur.execute("SELECT COALESCE(SUM(precio*cantidad),0) AS total FROM formulario WHERE fecha >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp AND fecha < (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp + interval '1 day' AND metodo_pago='Tarjeta'")
...
def obtener_notificaciones(current_user):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT DISTINCT ticket_id, cliente FROM formulario WHERE estado='Terminado' AND fecha >= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp AND fecha < (CURRENT_TIMESTAMP AT TIME ZONE 'America/Mazatlan')::date::timestamp + interval '1 day' LIMIT 5")
            rows = cur.fetchall()
        return jsonify([dict(r) for r in rows]), 200
    finally:
        conn.close()
